...
用法:
  python selftest_scripts.py            # 全跑（等价 --selftest）
  python selftest_scripts.py SC-5       # 只跑某条
  python selftest_scripts.py --help     # usage（rc=0）
退出码: 0=全部通过；1=有用例失败；2=用法错误（未知用例号）
"""
import argparse
import re
import subprocess
...
PY = sys.executable
PKG_ROOT = HERE.parent                        # scripts/ → 开源包根（verify_bundle.py 同口径）
...
def sc1_3(tmp: Path):
    ...
    write(a, "# 作品名称: 候选药物 X 脂质体研究\n\n作者: 张三\n\n测得粒径 45.2±5.6 nm。\n")
    write(b, "# 作品名称: 候选药物 X 脂质体研究\n\n作者: 张三\n\n测得粒径 45.2±5.6 nm。\n")
    write(c, "# 作品名称: 候选药物 X 脂质体研究\n\n作者: 张三\n\n测得粒径 99.9±1.1 nm。\n")
...
def sc4_5(tmp: Path):
    ...
    # SC-5：整句复制 → 必须 exit 1
    sent = "本研究以候选药物 X 为模型药物，采用薄膜分散法制备脂质体，并系统评价其体外释放行为。"
    write(target, f"## 引言\n\n{sent}\n")
    write(ref, f"## 引言\n\n{sent}\n")
    ...
    t2 = "本研究以候选药物 X 为模型药物，采用薄膜分散法制备脂质体，并系统评价其体内释放特征。"
    write(target, f"## 引言\n\n{t2}\n")
...
def _env_with_root(root: Path):
    """给被检脚本注入仓根（与 verify_bundle.py 的环境变量口径一致）。"""
    import os
    env = dict(os.environ)
    env["LABFLOW_ROOT"] = str(root)
    return env


def main(argv=None) -> int:
    known = ["SC-1", "SC-2", "SC-3", "SC-4", "SC-5", "SC-6"]
    ap = argparse.ArgumentParser(prog="selftest_scripts.py",
                                 description="校验器自检：证明 consistency_check / precheck_similarity / verify_bundle 真的能失败")
    ap.add_argument("case", nargs="?", help="只跑某条用例（%s）；缺省全跑" % " / ".join(known))
    ap.add_argument("--selftest", action="store_true", help="等价于无参全跑（统一各脚本的 --selftest 口径）")
    a = ap.parse_args(argv)
    only = a.case
    if only and only not in known:
        print("用法错误：未知用例号 %r（可用：%s）" % (only, " / ".join(known)))
        return 2
    print("=" * 60)
    print("多赛道 · 校验器自检（证明校验器真的能失败）")
    print("=" * 60)
    with tempfile.TemporaryDirectory(prefix="selftest_scripts_") as td:
        tmp = Path(td)
